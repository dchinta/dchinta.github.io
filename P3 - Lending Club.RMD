...
ggplot(aes(x=interest), data=subset(lc,lc$term != ""))+geom_histogram(binwidth =0.01,fill="orange")+scale_x_continuous(breaks=seq(0,0.3,0.02))+
  labs(list(title="Histogram of Interest Rates", x= "Interest rates", y ="Frequency"))+facet_wrap(~term)



```

There are higher number of loans at 36 month term than at 60 month term. Plot shows higher interest rates for 60 month term as is expected for longer term loans.      


```{r Faceted by Loan grades}
ggplot(aes(x=interest), data= subset(lc, lc$grade!=""))+geom_histogram(binwidth =0.01,fill="yellow")+scale_x_continuous(breaks=seq(0,0.3,0.03))+
  labs(list(title="Interest rates faceted by Loan grade", x= "Interest rates", y ="Frequency"))+facet_wrap(~grade)
...
ggplot(aes(x=annual_inc), data=lc)+geom_histogram(fill="light blue", binwidth = 25000)+xlim(0,300000)+geom_freqpoly(color= "red", binwidth=25000)
...
hist_loanamnt <- ggplot(aes(x=loan_amnt), data=lc)+
  ...
  labs(list(title="Histogram of Loan Amount", x= "Loan Amount", y ="Frequency"))
...
ggplot(aes(x=loan_status), data=subset(lc,!(lc$loan_status=="")))+geom_bar(fill="gold")+
  labs(list(title="Bar chart Loan status", x= "log10(Loan Status)", y ="Frequency"))+coord_flip()+scale_y_log10()

good_loans <- nrow(subset(lc, (loan_status == 'Current')|(loan_status == 'Fully Paid')))/nrow(subset(lc,!(lc$loan_status=="")))
...
state_choropleth(loan_vol, num_colors=8)
...
ggplot(aes(x=purpose), data=subset(lc,!((lc$purpose =="")|(lc$annualinc.cut == ""))))+geom_bar(fill="gold")+coord_flip()+scale_y_log10()+
...
Taking a closer look at interest rates and how loan grades are classified. Teh interest variable is cut into loan grades in lower case "a-g" corresposnding the to the interest rates published on Lending Club website. A scatte rplot for both the loan grades is plotted for comparison. 
...
ggplot(aes(x=grade,y=interest, colour=grade), data=subset(lc, lc$grade !=""))+geom_point()+scale_colour_brewer(palette="Set1")+
  scale_y_continuous(breaks=seq(0,0.3,0.02))

ggplot(aes(x=grade,y=interest, colour=int.cut), data=lc)+geom_point()+scale_colour_brewer(palette="Set1")+
  scale_y_continuous(breaks=seq(0,0.3,0.02))
``` 
...
ggplot(aes(x=round(annual_inc/1000)*1000, y = interest), data=lc)+geom_point(alpha=0.05, color="gold")+
  geom_line(stat='summary',fun.y=mean, color="red")+ylim(0,0.3)+xlim(0,300000)+xlab("annual income")
``` 
...
ggplot(aes(x=round(annual_inc/1000)*1000, y = dti), data=lc)+
  geom_point(alpha=0.05, color="gold")+geom_line(stat='summary',fun.y=mean, color="red")+ylim(0,50)+xlim(0,350000)+xlab("annual income")
...
ggplot(aes(x=annual_inc, y =loan_amnt, colour=grade), data=subset(lc,lc$grade != ""))+
  geom_point()+coord_cartesian(xlim=c(0,300000)) 
...
##plot.int1 <- ggplot(aes(x=emp_length, y =emp_int), data=emp.int)+geom_point()
...
prncp <- ggplot(aes(x=loan_amnt, y =total_rec_prncp, color=loan_status), data=subset(lc,!((lc$loan_status=="Fully Paid")|(lc$loan_status=="Current")|(lc$loan_status==""))))+
  geom_point()

prncp.int <- ggplot(aes(x=loan_amnt, y =total_rec_int+total_rec_prncp, color=loan_status), data=subset(lc,!((lc$loan_status=="Fully Paid")|(lc$loan_status=="Current")|(lc$loan_status==""))))+
  geom_point()
...
ggplot(aes(y=dti,x=loan_amnt,color=annualinc.cut), data=bl )+geom_point()+xlim(0,35000)+ylim(0,40)+geom_abline(aes(intercept = 0, slope = 40/35000))


ggplot(aes(y=dti, x = round(loan_amnt/10)*10), data=bl)+geom_point(stat = "summary", fun.y=mean)+ylim(0,50)


```


Exploring further on other parameters aside interest rates on what characterizes a bad loan, the box plot above shows a dense distribution of loans between DTI 10-35.The scatter plot shows that borrowers of lesser annual income, even at lesser loan amounts have higher DTI. On smoothing the scatter plot by using a smoothing function, we can see that at higher and lower loan amount of <10000 and > 25000, the variance in DTI is markedly huge compared to the in between range of loan amounts.  
...
DTI and Interest rate are mildly correlated in this data. The higher income groups are more populated at lower DTI's and viceversa. The data shows that there are not many points beyond DTI > 40, which indicates that Lending Clud might have a loan criteria for approvals.  

```{r}
ggplot(aes(x=annual_inc, y =loan_amnt, colour=grade), data=subset(lc,lc$grade != ""))+geom_point()+coord_cartesian(xlim=c(0,300000))+
  labs(list(title="Annual Income vs Loan Amount", x= "Annual Income", y ="Loan Amount"))
...
ggplot(aes(x=round(loan_amnt/500)*500, y = interest), data=lc)+geom_point(alpha=0.05,color="yellow")+
  geom_line(stat='summary',fun.y=mean, color="red")+
  labs(list(title="Interest vs Loan Amount", x= "Annual Income", 
            y ="Loan Amount"))
